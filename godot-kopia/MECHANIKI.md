...
- Główny HUD ma dolną konsolę: duży pasek życia pośrodku, mniejsze paski alkoholu i nikotyny po bokach oraz ikonę torby otwierającą ten sam ekwipunek co `I`. Pieniądze, pojemnik i progresja pozostają w kompaktowym panelu informacyjnym u góry.

### MEC-023: Powiększona mapa

- Obszar świata ma 2800×1600 pikseli, a kamera korzysta z tych samych granic.
- Mapa jest zapisana w `scenes/world_map.tscn`. Podłoże stanowi edytowalny `TileMapLayer`, natomiast pozycje obiektów rozgrywki są markerami widocznymi i przesuwalnymi w edytorze 2D Godota.
...
- Na mapie znajduje się 30 stałych puszek oraz 4 niezależne grupy kontenerów z osobnym losowaniem zawartości.
- Za płotem w lewym dolnym narożniku znajduje się zaniedbany park. Trzy polany zajmują watahy liczące kolejno 2, 3 i 4 dzikie psy; przy każdej grupie leżą po dwie puszki, więc wejście w głąb parku łączy ryzyko z nagrodą.
...
- Lewy przycisk myszy na przeciwniku wybiera cel. Bohater podchodzi automatycznie, jeśli cel jest poza zasięgiem, a następnie wykonuje atak za 10 obrażeń.
- Prawy przycisk myszy pozostaje dodatkowym 8-klatkowym atakiem obszarowym reklamówką.
- Ruch przypomina rzut młotem: szeroki zamach, maksymalne wyprostowanie torby i wyhamowanie. Nie obraca gotowego sprite'a programowo.
- Hitbox jest aktywny tylko w środkowej części animacji, ma zasięg 112 px i trafia każdy cel najwyżej raz na użycie.
- Hitbox korzysta z osobnej warstwy celu, dlatego nie uderza przedmiotów, sklepów ani postaci niewalczących.
- Zamach zadaje 8 obrażeń i pokazuje błysk trafienia.
- Wszystkie trzy watahy korzystają z jednego archetypu `park_dog`: poziom 1, 30 HP, atak 5, zasięg 1 i nagroda 50 XP za każdego psa.
...
- Szczór ma poziom 1, 12 HP, atak 2, brak obrony, zasięg 1 i nagrodę 8 XP. Zwykły atak bohatera pokonuje go po dwóch trafieniach.
- Wykrywa bohatera z 220 px i porusza się z prędkością 105 px/s. Ugryzienie wymaga podejścia na 48 px, ma 0,38 s czytelnej zapowiedzi i 1,60 s przerwy między próbami.
- Nie nakłada trucizny ani krwawienia. Jego rolą jest nauczenie gracza rozpoznawania przygotowania ataku i wybierania małego celu.
