...
signal collected(item_id: String, cash_cents: int)
signal out_of_range
...
func _ready() -> void:
	...
	highlight_sprite.texture = item_texture
	highlight_sprite.scale = Vector2.ONE * 0.070
	...
	sprite.scale = Vector2.ONE * 0.060
...
func interaction_prompt() -> String:
	return "ENTER  —  PODNIEŚ %s" % display_name.to_upper()
...
func try_collect() -> bool:
	if already_collected or player == null or player.is_action_busy():
		return false
...
func _finish_collect() -> void:
	collected.emit(item_id, cash_cents)
	queue_free()

func _draw() -> void:
	if interaction_focused:
		draw_circle(Vector2.ZERO, 31.0, Color("#efb64730"))
		draw_arc(Vector2.ZERO, 32.0, 0.0, TAU, 28, Color("#f2cd72bb"), 2.0)
