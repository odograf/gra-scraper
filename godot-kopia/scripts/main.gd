...
const BAG_HUD_ICON := preload("res://assets/ui/bag_icon.svg")
...
var wild_dog_enemy: WildDogEnemy
var wild_dog_enemies: Array[WildDogEnemy] = []
var sewer_rat_enemies: Array[SewerRatEnemy] = []
...
var bottom_hud_panel: PanelContainer
var bag_hud_button: Button
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		if gameplay_active and not _modal_open() and active_fence == null and player != null and not player.is_action_busy():
			...
			if clicked_enemy != null:
				...
				clicked_world_position.y = clampf(clicked_world_position.y, 20.0, WorldMapScript.WORLD_SIZE.y - 20.0)
				player.set_move_target(clicked_world_position)
				get_viewport().set_input_as_handled()
		return
...
func _park_dog_at_position(world_position: Vector2) -> WildDogEnemy:
	var selected: WildDogEnemy
	var closest_distance := 64.0
	for dog in wild_dog_enemies:
		if dog == null or not is_instance_valid(dog) or dog.dead:
			continue
		var distance := dog.global_position.distance_to(world_position)
...
func _click_hits_interactable(world_position: Vector2) -> bool:
...
func _create_world() -> void:
	...
	if world_map == null:
		# Awaryjny wariant dla testów lub starych scen bez world_map.tscn.
		world_map = WorldMapScript.new() as WorldMap
		world_map.name = "Osiedle"
		world_map.y_sort_enabled = true
		add_child(world_map)
	var editor_markers := world_map.get_node_or_null("PunktyRozgrywki") as Node2D
	if editor_markers != null:
		# Markery i ich podglądy są widoczne w edytorze, a właściwe obiekty
		# rozgrywki są tworzone na ich pozycjach podczas uruchamiania.
		editor_markers.visible = false

func _map_marker_position(marker_path: String, fallback: Vector2) -> Vector2:
	if world_map == null:
		return fallback
	var marker := world_map.get_node_or_null("PunktyRozgrywki/" + marker_path) as Node2D
	return marker.global_position if marker != null else fallback

func _map_marker_folder(folder_path: String) -> Node:
	if world_map == null:
		return null
	return world_map.get_node_or_null("PunktyRozgrywki/" + folder_path)
...
func _create_world_interactions() -> void:
	...
	starter_bag.position = _map_marker_position("Interakcje/ReklamowkaStartowa", Vector2(815, 625))
	...
	machine.position = _map_marker_position("Interakcje/AutomatNaPuszki", Vector2(835, 430))
	machine.z_index = 6
	...
	door.position = _map_marker_position("Interakcje/WejscieDoSklepu", Vector2(2450, 450))
	door.z_index = 7
	door.player = player
	...
	kiosk.position = _map_marker_position("Interakcje/OkienkoKiosku", Vector2(1040, 520))
	kiosk.z_index = 7
	kiosk.player = player
	...
	combat_enemy.position = _map_marker_position("Interakcje/ZadymiarzPodKioskiem", Vector2(1160, 580))
	combat_enemy.z_index = 9
	combat_enemy.player = player
	combat_enemy.fight_requested.connect(_open_combat_prototype.bind("zadymiarz", null, combat_enemy))
	...
	mirek.name = "Mirek"
	mirek.position = _map_marker_position("Interakcje/Mirek", Vector2(1460, 610))
	mirek.z_index = 8
	mirek.player = player
	...
	zul_1.name = "Zul1"
	zul_1.position = _map_marker_position("StanyNPC/ZulCzeka", ZUL_WAIT_POSITION)
	zul_1.z_index = 8
	...
	burek.name = "Burek"
	burek.position = _map_marker_position("StanyNPC/BurekStart", BUREK_DOOR_POSITION)
	burek.z_index = 8
	...
	heniek.name = "Heniek"
	heniek.position = _map_marker_position("Interakcje/Heniek", Vector2(1210, 1335))
	heniek.z_index = 8
	...
	fence.position = _map_marker_position("Interakcje/SiatkaDoWyciecia", Vector2(490, 855))
	fence.z_index = 8
	fence.player = player
	...
	var trash_markers := _map_marker_folder("Kontenery")
	if trash_markers != null:
		for marker_node in trash_markers.get_children():
			var marker := marker_node as Marker2D
			if marker == null:
				continue
			var container := _create_trash_container(marker.name, marker.global_position, bool(marker.get_meta("guaranteed_wire", false)))
			if marker.name == "KonteneryStartowe":
				trash_bin = container
	else:
	...
	_create_park_dog_packs()
	_create_sewer_rats()

func _create_park_dog_packs() -> void:
	var packs := _map_marker_folder("WatahyPsow")
	if packs == null:
		return
	for pack_node in packs.get_children():
		for dog_marker_node in pack_node.get_children():
			var dog_marker := dog_marker_node as Marker2D
			if dog_marker == null:
				continue
			var pack_id := int(dog_marker.get_meta("pack_id", 1))
			var dog_index := int(dog_marker.get_meta("dog_index", 1))
			var dog := WildDogEnemyScript.new() as WildDogEnemy
			dog.name = "DzikiWychudzonyPies" if pack_id == 1 and dog_index == 1 else "DzikiPiesWataha%d_%d" % [pack_id, dog_index]
			dog.position = dog_marker.global_position
			dog.z_index = 8
			dog.player = player
...
func _on_park_dog_defeated(dog: WildDogEnemy) -> void:
	if dog == null:
		return
	var levels_gained := _award_xp(dog.xp_reward, "pokonanie psa z parku")
...
func _create_sewer_rats() -> void:
	var markers := _map_marker_folder("Szczory")
	if markers == null:
		return
	for marker_node in markers.get_children():
		...
		rat.name = marker.name
		rat.position = marker.global_position
		rat.z_index = 8
		rat.player = player
...
func _on_sewer_rat_defeated(rat: SewerRatEnemy) -> void:
	if rat == null:
		return
	var levels_gained := _award_xp(rat.xp_reward, "pokonanie Szczóra")
...
func _create_pickups() -> void:
	# Pierwsze osiem markerów gwarantuje FAB-01; pozostałe nagradzają eksplorację.
	var marker_folder := _map_marker_folder("Puszki")
	if marker_folder == null:
		return
	var can_markers := marker_folder.get_children()
	can_markers.sort_custom(func(a: Node, b: Node) -> bool:
		return int(a.get_meta("spawn_index", 0)) < int(b.get_meta("spawn_index", 0))
	)
	for marker_node in can_markers:
		var marker := marker_node as Marker2D
		if marker == null:
			continue
		var index := int(marker.get_meta("spawn_index", 0))
		var can := PickupCanScript.new()
		can.name = "Puszka%d" % (index + 1)
		can.position = marker.global_position
		can.rotation = float(marker.get_meta("rotation", 0.0))
		can.player = player
...
func _create_hud(canvas: CanvasLayer) -> void:
	...
	top_panel.name = "InformacjeBohatera"
	top_panel.position = Vector2(20, 20)
	top_panel.custom_minimum_size = Vector2(370, 116)
	...
	bag_hud_button.add_theme_stylebox_override("hover", _hud_bag_button_style(Color("#393a2d"), Color("#efc870")))
	bag_hud_button.add_theme_stylebox_override("pressed", _hud_bag_button_style(Color("#51452c"), Color("#efc870")))
	...
	if combat_result == "victory":
		...
		if active_combatant_node != null and is_instance_valid(active_combatant_node):
			...
			if active_combatant_id == "burek":
				...
				defeated_position = _map_marker_position("StanyNPC/ZulPoWalce", ZUL_DEFEATED_POSITION)
...
func _activate_zul_blockade() -> void:
	...
	approach.tween_property(zul_npc, "position", _map_marker_position("StanyNPC/ZulBlokuje", ZUL_BLOCK_POSITION), 0.65)
...
func _show_message(text: String, danger := false) -> void:
	...
	tween.tween_callback(func() -> void:
		if not _modal_open():
			message_label.text = "LPM na wrogu — atak  •  Strzałki / WASD — ruch  •  ENTER — akcja  •  I — ekwipunek"
...
func _hud_resource_box(minimum_width: float) -> VBoxContainer:
	var box := VBoxContainer.new()
	box.custom_minimum_size.x = minimum_width
	box.alignment = BoxContainer.ALIGNMENT_CENTER
	box.add_theme_constant_override("separation", 5)
	return box

func _hud_resource_label(title: String, color: Color) -> Label:
	var label := _label(title, 12, color)
...
func _hud_bar(fill_color: Color, height: int) -> ProgressBar:
	var bar := ProgressBar.new()
	bar.min_value = 0.0
	bar.max_value = 100.0
	bar.show_percentage = false
	...
	var fill := StyleBoxFlat.new()
	fill.bg_color = fill_color
	fill.border_color = fill_color.lightened(0.22)
