...
func _run() -> void:
	...
	_expect(rules.player_health == 100 and rules.enemy_health == 46, "Walka zaczyna się z pełnym zdrowiem obu stron")
	...
	dog_rules.configure_enemy_stats(CombatantConfigScript.enemy("burek"))
	dog_rules.reset(20260825)
	_expect(dog_rules.enemy_health == 20 and dog_rules.enemy_attack == 3, "Burek ma małe statystyki przeciwnika wprowadzającego")
	var zul_rules = CombatRulesScript.new()
	zul_rules.configure_enemy_stats(CombatantConfigScript.enemy("zul_1"))
	zul_rules.reset(20260825)
	_expect(zul_rules.enemy_health == 64 and zul_rules.enemy_attack == 10, "Żul 1 jest wyraźnie mocniejszym przeciwnikiem")
	...
	_expect(game.combat_enemy != null and game.combat_enemy.position == game._map_marker_position("Interakcje/ZadymiarzPodKioskiem", Vector2(1160, 580)), "Zadymiarz stoi pod kioskiem")
	...
	game.combat_overlay._choose_action("quick")
	...
	_expect(game.zul_defeated and zul_1.position.is_equal_approx(game._map_marker_position("StanyNPC/ZulPoWalce", game.ZUL_DEFEATED_POSITION)), "Pokonany Żul 1 odsuwa się z jedynego przejścia")
	game._close_combat_prototype()
	...
	game._on_combat_resolved("victory")
	await create_timer(0.75).timeout
	_expect(game.burek_defeated and burek.position.is_equal_approx(game._map_marker_position("StanyNPC/BurekPoWalce", game.BUREK_DEFEATED_POSITION)), "Pokonany Burek odsuwa się od drzwi sklepu")
	game._close_combat_prototype()
