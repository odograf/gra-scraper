...
func _run() -> void:
	...
	_expect(zul_1.position == game.ZUL_WAIT_POSITION and not zul_1.fightable, "Żul 1 czeka z boku do zakończenia zlecenia z siatką")
	_expect(burek.position == game.BUREK_DOOR_POSITION, "Burek stoi dokładnie w świetle drzwi Żuka Gnojarza")
