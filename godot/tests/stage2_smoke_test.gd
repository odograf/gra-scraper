...
func _run() -> void:
	...
	# Bohater używa wskazanego przez użytkownika, oryginalnego arkusza 3 na 4.
	...
	_expect(game.player.LOCOMOTION_SHEET.get_size() == Vector2(1288, 1221), "Przywrócony arkusz ma oryginalny rozmiar")
	_expect(game.player.LOCOMOTION_FRAME_SIZE == Vector2i(429, 305), "Arkusz jest dzielony na 3 kolumny i 4 kierunki")
	_expect(game.player.LOCOMOTION_SHEET.resource_path.ends_with("collector_walk_sheet_v2.png"), "Bohater używa dokładnie wskazanego starego sprite'a")
	...
	_expect(game.zul_npc.position.is_equal_approx(game.ZUL_BLOCK_POSITION), "Żul 1 wchodzi w jedyne przejście do Mirka")
	game._open_mirek()
